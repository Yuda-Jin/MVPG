...
# ============================================================================
# Legacy-compat layer: official FG-CLIP2 modeling targets transformers ~4.53+,
# this project pins transformers 4.34 (required by LLaVA-1.5). The shims below
# reproduce the few new-API pieces actually used at runtime.
# ============================================================================
...
def _prepare_4d_attention_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None) -> torch.Tensor:
    """Same semantics as transformers.modeling_attn_mask_utils._prepare_4d_attention_mask (4.36+)."""
    if tgt_len is None:
        tgt_len = mask.shape[-1]
    expanded_mask = mask[:, None, None, :].to(dtype=dtype)
    expanded_mask = expanded_mask.expand(mask.shape[0], 1, tgt_len, mask.shape[-1])
    inverted_mask = 1.0 - expanded_mask
    inverted_mask = inverted_mask.masked_fill(inverted_mask.to(torch.bool), torch.finfo(dtype).min)
    return inverted_mask


# 4.53+'s `GradientCheckpointingLayer` only matters when gradient checkpointing is
# requested; inference never goes through the parent class hooks, so nn.Module suffices.
class GradientCheckpointingLayer(nn.Module):
    pass


# Kept only for reference / non-eager fallback; we always force `_attn_implementation="eager"`.
...
class TransformersKwargs(TypedDict):
    """TypedDict stand-in for `transformers.utils.TransformersKwargs` (PEP 692 annotation)."""

# --- no-op decorators (docstring generation / tuple-return / signature filtering).
# 兼容两种写法：@auto_docstring（裸用）与 @auto_docstring(custom_intro=...)（带参调用）。
...
def can_return_tuple(obj=None, *args, **kwargs):
    if callable(obj):
        return obj
    def _decorator(o):
        return o
...
def filter_out_non_signature_kwargs(obj=None, *args, **kwargs):
    if callable(obj):
        return obj
    def _decorator(o):
        return o
...
def check_model_inputs(obj=None, *args, **kwargs):
    if callable(obj):
        return obj
    def _decorator(o):
        return o
